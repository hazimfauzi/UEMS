...
if(isset($_POST['submit'])){
	if(($_FILES['eventBanner']['type'] = 'image/gif') || ($_FILES['eventBanner']['type'] = 'image/jpeg') || ($_FILES['eventBanner']['type'] = 'image/pjpeg') && ($_FILES['eventBanner']['size'] < 2000000))
	{
		...
		else if (move_uploaded_file($_FILES['eventBanner']['tmp_name'],'../../upload/'.$_FILES['eventBanner']['name']))
		{
			...
			$sql = mysql_query("insert into event(eventTitle, eventCategory, eventLocation, startTime, endTime, startDate, endDate, maxParticipant,
												 eventBanner, eventDescription, eventStatus, createdBy) 
								values('$eventTitle', '$eventCategory', '$eventLocation', '$startTime', '$endTime', '$startDate', '$endDate', 			
								       '$maxParticipant','$part', '$eventDescription', 'Pending','$session_id')
							   ");
	if($sql)
	{
